...
;; (helper_fst (cons (cons (car lst) '()) '()) (cdr lst) lst count))
;; Definition:
(define (lis_fast lst)
  (define finallist '())
  (define count 1)
  (cond
   ((not(null? lst))(finallistedit (helper_fst (cons (cons (car lst) '()) '()) (cdr lst) lst count) finallist (car (helper_fst (cons (cons (car lst) '()) '()) (cdr lst) lst count))))
   ((null? lst) lst)
   )    
)
...
;;Contract: finallisted: lst finallst currentval-> finallst
;;Purpose: Handle situations in which the subsequence is broken up by a greater value
;;lst, the list after trying to find the longest subsequence
;;finallist, the edited list
;;currentval, the currentsmallest value
;;Example (finallistedit (7 6 2 4 3 2 1) () 7)-> (1 2 2 2 2 6 7)
;;Definition: 
(define (finallistedit lst finallst currentval)
   (cond
     ((not (null? lst))(editor lst finallst currentval))
     ((null? lst) finallst)
     )
)

;;Contract: editor: lst finallst currentval-> (finallistedit (cdr lst)(finallist+lowestval) lowestval)
;;Purpose: check if the next val in lst is smaller than current smallest if not add currentval to
;;finallist otherwise add next val to finallist
;;lst; list after trying to find longest subsequence
;;finallist; current edited list to be returned
;;currentval; current smallest value
;;Example: (editor (3 2 1) (2 2 6 7) 2 )-> (finallistedit (2 1) (2 2 2 6 7) 2)
;;Definition
(define (editor lst finallst currentval)
  (cond
   ((equal? currentval (car lst)) (finallistedit (cdr lst) (cons (car lst) finallst) (car lst)) )
   ((> currentval (car lst)) (finallistedit (cdr lst) (cons (car lst) finallst) (car lst)))
   ((< currentval (car lst)) (finallistedit (cdr lst) (cons currentval finallst) currentval) )
   )
  )
...
(define list3 '())
(define list4 '(1 2 3 ))
(define list5 '(1 2 3 2 4 1 2 2 6 4 7 1))
